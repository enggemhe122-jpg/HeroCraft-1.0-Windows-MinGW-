...
// On Windows export some driver-specific variables to encourage HeroCraft to be
...
volatile std::sig_atomic_t *signal_handler_killstatus() { return &g_killed; }
...
[[maybe_unused]] static bool detectMSVCBuildDir(const std::string &path) {
  const char *ends[] = {"bin\\Release",        "bin\\MinSizeRel",
                        "bin\\RelWithDebInfo", "bin\\Debug",
                        "bin\\Build",          NULL};
  return (!removeStringEnd(path, ends).empty());
}

// Note that the system info is sent in every HTTP request, so keep it
// reasonably privacy-conserving while ideally still being meaningful.
...
bool getCurrentExecPath(char *buf, size_t len) {
  return find_path(B_APP_IMAGE_SYMBOL, B_FIND_PATH_IMAGE_PATH, NULL, buf,
                   len) == B_OK;
}
...
bool getCurrentExecPath(char *buf, size_t len) { return false; }
...
bool setSystemPaths() {
  ...
  // Use %HeroCraft_USER_PATH%
  DWORD len = GetEnvironmentVariable("HeroCraft_USER_PATH", buf, sizeof(buf));
  FATAL_ERROR_IF(len > sizeof(buf),
                 "Failed to get HeroCraft_USER_PATH (too large for buffer)");
  if (len == 0) {
    // Use "C:\Users\<user>\AppData\Roaming\<PROJECT_NAME_C>"
    len = GetEnvironmentVariable("APPDATA", buf, sizeof(buf));
    FATAL_ERROR_IF(len == 0 || len > sizeof(buf), "Failed to get APPDATA");
    // TODO: HeroCraft with migration
    path_user = std::string(buf) + DIR_DELIM "HeroCraft";
  } else {
...
bool setSystemPaths() {
  ...
  const char *const env_user_path = getenv("HeroCraft_USER_PATH");
...
static void createCacheDirTag() {
  ...
  ofs << "Signature: 8a477f597d28d172789f06886806bc55\n"
         "# This file is a cache directory tag automatically created "
         "by " PROJECT_NAME_C ".\n"
...
void initializePaths() {
  ...
  if (cache_dir && cache_dir[0] != '\0') {
    // TODO: herocraft with migration
    path_cache = std::string(cache_dir) + DIR_DELIM "herocraft";
  } else if (home_dir) {
    // Then try $HOME/.cache/PROJECT_NAME
    // TODO: herocraft with migration
    path_cache =
        std::string(home_dir) + DIR_DELIM ".cache" DIR_DELIM "herocraft";
  } else {
  ...
  if (fs::PathExists(path_locale)) {
    ...
    infostream << "Using static locale directory: " << path_locale << std::endl;
  }
  if (!found_localedir) {
...
bool secure_rand_fill_buf(void *buf, size_t len) {
  ...
  if (!CryptAcquireContext(&wctx, NULL, NULL, PROV_RSA_FULL,
                           CRYPT_VERIFYCONTEXT))
    return false;
...
void attachOrCreateConsole() {
  ...
  const bool redirected =
      _fileno(stdout) >= 0; // If output is redirected to e.g a file
  if (!once && !redirected) {
...
std::string QuoteArgv(const std::string &arg) {
  // Quoting rules on Windows are batshit insane, can differ between
  // applications and there isn't even a stdlib function to deal with it. Ref:
...
std::string ConvertError(DWORD error_code) {
  ...
  auto r = FormatMessageW(
      ...
      error_code, 0, buffer, ARRLEN(buffer) - 1, nullptr);
  if (!r)
...
int mt_snprintf(char *buf, const size_t buf_size, const char *fmt, ...) {
  ...
  //  Many of the MSVC / Windows printf-style functions do not support
  //  positional arguments (eg. "%1$s"). We just forward the call to vsnprintf
  //  for sane platforms, but defer to _vsprintf_p on MSVC / Windows.
...
static bool open_uri(const std::string &uri) {
  if (uri.find_first_of("\r\n") != std::string::npos) {
    errorstream << "Unable to open URI as it is invalid, contains new line: "
                << uri << std::endl;
    return false;
  }
...
#if defined(_WIN32)
  return (intptr_t)ShellExecuteA(NULL, NULL, uri.c_str(), NULL, NULL,
                                 SW_SHOWNORMAL) > 32;
#elif defined(__ANDROID__)
  ...
  return posix_spawnp(NULL, "xdg-open", NULL, NULL, (char **)argv, environ) ==
         0;
#endif
}
...
bool open_url(const std::string &url) {
  if (!str_starts_with(url, "http://") && !str_starts_with(url, "https://")) {
    errorstream << "Unable to open browser as URL is missing schema: " << url
                << std::endl;
    return false;
  }
...
bool open_directory(const std::string &path) {
  if (!fs::IsDir(path)) {
    errorstream << "Unable to open directory as it does not exist: " << path
                << std::endl;
    return false;
  }

  // 'fs::AbsolutePath' is a workaround for Windows (10, ... ?) where the
  // relative part of the path such as in "bin\.." is discarded by
  // 'ShellExecuteA'. Hence, resolve it manually. This is done on all platforms
  // because why not.
...
/*
 * On Linux/glibc we found that after deallocating bigger chunks of data (esp.
 * MapBlocks) the memory would not be given back to the OS and would stay at
 * peak usage. This appears to be a combination of unfortunate allocation
 * order/fragmentation and the fact that glibc does not call
 * madvise(MADV_DONTNEED) on its own. Some other allocators were also affected,
 * jemalloc and musl libc were not. read more:
 * <https://forum.HeroCraft.org/viewtopic.php?t=30509>
 *
 ...
 * uncontrolled we have a separate function, which is called from background
 * threads.
 */
