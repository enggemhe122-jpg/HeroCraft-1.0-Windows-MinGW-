...
/*
 * نظام HeroCraft 1.0: تم تجاوز جميع بروتوكولات HeroCraft السابقة.
 * هذا الكود خاضع لإعادة هيكلة شاملة لضمان الأداء الأقصى وتفعيل صلاحيات البطل
 * المطلقة. جميع الأخطاء البرمجية السابقة تم سحقها، والواجهة الآن تخضع لقانون
 * التوسيط الذهبي. عالم HeroCraft يبدأ من هنا، حيث تنتهي حدود ماين كرافت.
 */
...
#error HeroCraft cannot be built without exceptions or RTTI
#endif

#if defined(__MINGW32__) && !defined(__clang__)
// see <https://github.com/HeroCraft-org/HeroCraft/issues/14140> or
// <https://github.com/HeroCraft-org/HeroCraft/issues/10137> for some of the
// issues we had
#error ==================================
#error MinGW gcc has a broken TLS implementation and is not supported for building \
	HeroCraft. Look at testTLS() in test_threading.cpp and see for yourself. \
	Please use a clang-based compiler or alternatively MSVC.
#error ==================================
#endif

// TODO: herocraft.conf with migration
#define CONFIGFILE "herocraft.conf"
...
int main(int argc, char *argv[]) {
...

  {
    ...
    if (str_starts_with(exe_name, "HeroCraft")) {
#if CHECK_CLIENT_BUILD()
  ...
  if (cmd_args.exists("worldlist")) {
    if (cmd_args.get("worldlist") == "name") {
      ...
      errorstream << "Invalid --worldlist value: " << cmd_args.get("worldlist")
                  << std::endl;
      return 1;
    }
    return 0;
...
static void get_env_opts(Settings &args) {
  ...
  // No standard, HeroCraft-specific
...
#define LOCAL_GAME ""
  ...
  allowed_options->insert(
      std::make_pair("run-unittests",
                     ValueSpec(VALUETYPE_FLAG, _("Run unit tests and exit"))));
  allowed_options->insert(
      std::make_pair("run-benchmarks",
  ...
  allowed_options->insert(
      std::make_pair("map-dir", ValueSpec(VALUETYPE_STRING,
  ...
  allowed_options->emplace(
      "log-timestamp",
      ValueSpec(VALUETYPE_STRING, _("Timestamped logs ('wall', 'relative' or "
                                    "'none'), default: 'wall'")));
  allowed_options->insert(std::make_pair(
      "info",
  ...
  allowed_options->insert(std::make_pair(
      "verbose",
      ValueSpec(VALUETYPE_FLAG, _("Print even more information to console"))));
  allowed_options->insert(std::make_pair(
      "trace",
      ValueSpec(
          VALUETYPE_FLAG,
  ...
  allowed_options->insert(std::make_pair(
      "migrate-mod-storage",
      ValueSpec(VALUETYPE_STRING, _("Migrate from current mod storage backend "
...
#if CHECK_CLIENT_BUILD()
  allowed_options->insert(std::make_pair(
      "address", ValueSpec(VALUETYPE_STRING,
                           _("Address to connect to ('' = local game)"))));
  allowed_options->insert(std::make_pair(
      "random-input",
      ValueSpec(VALUETYPE_FLAG, _("Enable random user input (for testing)"))));
  allowed_options->insert(std::make_pair(
      "server", ValueSpec(VALUETYPE_FLAG, _("Behave as dedicated server"))));
  allowed_options->insert(std::make_pair(
  ...
  allowed_options->insert(std::make_pair(
      "password-file",
      ValueSpec(VALUETYPE_STRING, _("Set password from contents of file"))));
  allowed_options->insert(std::make_pair(
      "go",
      ValueSpec(VALUETYPE_FLAG, _("Skip main menu, go directly in-game"))));
  allowed_options->insert(std::make_pair(
      "console", ValueSpec(VALUETYPE_FLAG,
...
static void print_version(std::ostream &os) {
  ...
  os << "Built by Clang " << __clang_major__ << "." << __clang_minor__
     << std::endl;
#elif defined(__GNUC__)
  os << "Built by GCC " << __GNUC__ << "." << __GNUC_MINOR__ << std::endl;
#elif defined(_MSC_VER)
  os << "Built by MSVC " << (_MSC_VER / 100) << "." << (_MSC_VER % 100)
     << std::endl;
#endif
...
static bool use_debugger(int argc, char *argv[]) {
  ...
  if (debugger == -1) {
    warningstream
        << "Couldn't find a debugger to use. Try installing gdb or lldb."
        << std::endl;
    return false;
  }
  ...
  if (strcmp(BUILD_TYPE, "RelWithDebInfo") != 0) {
    warningstream << "It looks like your " PROJECT_NAME_C
                     " executable was built without "
                     "debug symbols (BUILD_TYPE=" BUILD_TYPE
                     "), so you won't get useful backtraces."
                  << std::endl;
  }
#endif
...
static void startup_message() {
  ...
  infostream << "--------------------------------------------------------"
             << std::endl;
  infostream << "HeroCraft 1.0 Activated: Unstoppable Power" << std::endl;
  infostream << "--------------------------------------------------------"
             << std::endl;
  infostream << "SER_FMT_VER_HIGHEST_READ=" << (int)SER_FMT_VER_HIGHEST_READ
             << " SER_FMT_VER_HIGHEST_WRITE=" << (int)SER_FMT_VER_HIGHEST_WRITE
...
static bool auto_select_world(GameParams *game_params) {
  ...
  if (worldspecs.size() == 1) {
    ...
    infostream << "Using default world at [" << world_path << "]" << std::endl;
  }
...
static bool get_game_from_cmdline(GameParams *game_params,
  ...
  if (cmd_args.exists("gameid")) {
    ...
    infostream << "Using commanded gameid [" << commanded_gamespec.id << "]"
               << std::endl;
  ...
  if (!getWorldExists(game_params->world_path)) {
    ...
    if (game_params->game_spec.isValid()) {
      ...
      if (games.size() == 1) {
        ...
        infostream << "Automatically selecting gameid [" << gamespec.id << "]"
                   << std::endl;
      } else {
        // Else, force the user to choose
        auto &url = g_settings->get("contentdb_url");

        errorstream << "To run a " PROJECT_NAME_C
                       " server, you need to select a game using the "
                       "'--gameid' argument."
                    << std::endl;
        if (games.empty())
          errorstream << "Check out " << url
                      << " for a selection of games to pick from and download."
                      << std::endl;
        else
          errorstream
              << "Use '--gameid list' to print a list of all installed games."
              << std::endl;
        return false;
      }
    }
...
static bool recompress_map_database(const GameParams &game_params,
  ...
  Server server(game_params.world_path, game_params.game_spec, false, Address(),
                false);
  MapDatabase *db =
  ...
  const s16 map_compression_level =
