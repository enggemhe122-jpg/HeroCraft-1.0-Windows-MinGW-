	...
	m_server(server)
{
	...
	m_step_time_counter = mb->addCounter(
		"HeroCraft_env_step_time", "Time spent in environment step (in microseconds)");

	m_active_block_gauge = mb->addGauge(
		"HeroCraft_env_active_blocks", "Number of active blocks");

	m_active_object_gauge = mb->addGauge(
		"HeroCraft_env_active_objects", "Number of active objects");
}
