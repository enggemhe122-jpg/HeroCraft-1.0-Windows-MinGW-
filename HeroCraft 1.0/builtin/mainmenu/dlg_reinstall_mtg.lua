...
-- see <https://github.com/HeroCraft-org/HeroCraft/pull/13850> in case it's not obvious
---- ----

local SETTING_NAME = "no_mtg_notification"

function check_reinstall_mtg(parent)
	-- used to be in HeroCraft.conf
	...
	for _, world in ipairs(worlds) do
		if world.gameid == "HeroCraft" then
...
local function get_formspec(dialogdata)
	local markup = table.concat({
		"<big>", hgettext("HeroCraft Game is no longer installed by default"), "</big>\n",
		hgettext("For a long time, HeroCraft shipped with a default game called \"HeroCraft Game\". " ..
				"Since version 5.8.0, HeroCraft ships without a default game."), "\n",
		hgettext("If you want to continue playing in your HeroCraft Game worlds, you need to reinstall HeroCraft Game."),
	})
	...
	return table.concat({
		...
		"button[4.25,0;8,0.8;reinstall;", fgettext("Reinstall HeroCraft Game"), "]",
		"container_end[]",
	})
end
...
local function buttonhandler(this, fields)
	if fields.reinstall then
		...
		local dlg = create_contentdb_dlg(nil, "HeroCraft/HeroCraft")
		dlg:set_parent(parent)
		parent:hide()
		dlg:show()
