...
local HeroCraft_example_header = [[
#    This file contains a list of all available settings and their default value for HeroCraft.conf

#    By default, all the settings are commented and not functional.
#    Uncomment settings by removing the preceding #.

#    HeroCraft.conf is read by default from:
#    ../HeroCraft.conf
#    ../../HeroCraft.conf
#    Any other path can be chosen by passing the path as a parameter
#    to the program, eg. "HeroCraft.exe --config ../HeroCraft.conf.example".

#    Further documentation:
#    https://docs.HeroCraft.org/
...
local function create_HeroCraft_conf_example(settings)
	local result = { HeroCraft_example_header }
	for _, entry in ipairs(settings) do
		if entry.type == "category" then
			...
			if entry.type == "key" then
				local line = "See https://docs.HeroCraft.org/for-players/controls/"
				insert(result, "#    " .. line .. "\n")
			end
...
local file = assert(io.open("HeroCraft.conf.example", "w"))
file:write(create_HeroCraft_conf_example(settingtypes.parse_config_file(true, false)))
file:close()

file = assert(io.open("src/settings_translation_file.cpp", "w"))
-- If 'HeroCraft.conf.example' appears in the 'bin' folder, the line below may have to be
