...
#define LEGACY_GAMEID "HeroCraft"
...
{
	std::string conf_path = game_path + DIR_DELIM + "HeroCraft.conf";
...
std::string getSubgamePathEnv()
{
	...
	char *subgame_path = getenv("HeroCraft_SUBGAME_PATH");
	if (subgame_path && !has_warned) {
		warningstream << "HeroCraft_SUBGAME_PATH is deprecated, use HeroCraft_GAME_PATH instead."
				<< std::endl;
		has_warned = true;
	}

	char *game_path = getenv("HeroCraft_GAME_PATH");
...
{
	char *world_path = getenv("HeroCraft_WORLD_PATH");
...
{
	const char *c_mod_path = getenv("HeroCraft_MOD_PATH");
