...
/*
 * inspired by
 * https://github.com/systemd/systemd/blob/7aed43437175623e0f3ae8b071bbc500c13ce893/src/hostname/hostnamed.c#L406
...
static bool detect_touch() {
  ...
  if (dtb_file.is_open()) {
    ...
    if (chassis_type == "tablet" || chassis_type == "handset" ||
        chassis_type == "watch")
      return true;
...
void set_default_settings() {
  ...
  settings->setDefault("serverlist_url", "https://servers.herocraft.org");
  ...
  // see <https://github.com/HeroCraft-org/HeroCraft/issues/12792>
  settings->setDefault("keymap_rangeselect",
                       has_touch ? "SYSTEM_SCANCODE_21" : ""); // KEY_KEY_R
  ...
  settings->setDefault("keymap_hotbar_previous",
                       "SYSTEM_SCANCODE_5");                 // KEY_KEY_B
  ...
  settings->setDefault("texture_min_size",
                       std::to_string(TEXTURE_FILTER_MIN_SIZE));
  ...
  settings->setDefault(
      "font_path", porting::getDataPath("fonts" DIR_DELIM "Arimo-Regular.ttf"));
  settings->setDefault(
      "font_path_italic",
  ...
  // ContentDB
  settings->setDefault("contentdb_url", "https://content.herocraft.org");
  ...
  settings->setDefault("update_information_url",
                       "https://www.herocraft.org/release_info.json");
#else
  settings->setDefault("update_information_url", "");
#endif
  ...
  settings->setDefault("kick_msg_crash",
                       "This server has experienced an internal error. You "
  ...
  settings->setDefault("enable_remote_media_server", "true");
  settings->setDefault("remote_media_whitelist", "https://media.herocraft.org");
  settings->setDefault("strict_protocol_version_checking", "false");
  settings->setDefault("enable_rollback_recording", "true");
  settings->setDefault("server_map_save_interval", "5");
  settings->setDefault("sqlite_synchronous", "0");
  settings->setDefault("max_users", "1000");
  settings->setDefault("default_privs", "interact, shout, fly, fast");
