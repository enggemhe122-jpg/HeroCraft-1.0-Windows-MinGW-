	...
	m_modchannel_mgr(new ModChannelMgr())
{
	...
	m_uptime_counter = m_metrics_backend->addCounter("HeroCraft_core_server_uptime", "Server uptime (in seconds)");
	m_player_gauge = m_metrics_backend->addGauge("HeroCraft_core_player_number", "Number of connected players");

	m_timeofday_gauge = m_metrics_backend->addGauge(
			"HeroCraft_core_timeofday",
			"Time of day value");

	m_lag_gauge = m_metrics_backend->addGauge(
			"HeroCraft_core_latency",
...
ModStorageDatabase *Server::openModStorageDatabase(const std::string &world_path)
{
	...
	if (backend == "files") {
		warningstream << "/!\\ You are using the old mod storage files backend. "
			...
			"please read https://docs.HeroCraft.org/for-server-hosts/database-backends." << std::endl;
	}
