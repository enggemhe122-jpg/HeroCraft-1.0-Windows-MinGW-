...
function pkgmgr.get_contentdb_id(content)
	...
	-- Until version 5.8.0, HeroCraft Game was bundled with the engine.
	...
	if content.type == "game" and content.id == "HeroCraft" and content.release == 0 and
			not core.is_dir(content.path .. "/.git") and core.may_modify_path(content.path) then
		return "HeroCraft/HeroCraft"
	end
